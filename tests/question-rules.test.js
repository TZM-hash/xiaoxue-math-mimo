...
function runHangzhouCurriculumMetadataTests() {
  ...
  assert.strictEqual(debug.curriculumSelectShortLabel(debug.pointMap["g2-100-add"]), "二上 · 100以内加减 · 100以内", "selector short labels should keep term and compact unit context");
  assert.strictEqual(debug.curriculumSelectShortLabel(debug.pointMap["g2-table-div"]), "二下 · 表内除法 · 表内除", "selector short labels should keep second-term division context concise");
  ...
  assert(grade2Options.includes(">二下 · 表内除法 · 表内除</option>"), "selector options should show compact textbook-aware labels");
