(function () {
  ...
  // 原创奥数题：独立补充四年级数学思维训练，不指向任何参考资料页。
  add("g4-thinking", [
    text("orig-g4-olympiad-001", "甲乙两数的和是 156，甲数比乙数的 3 倍少 4。乙数是多少？", "40", "设乙为 x，甲为 3x - 4，4x - 4 = 156，x = 40。", ["设较小数为 x。", "根据倍数关系列式。", "解出 x。"], "原创奥数-和倍问题", "原创奥数题，与参考资料派生题隔离。"),
    choice("orig-g4-olympiad-002", "找规律：1，4，9，16，25，下一个数是？", "36", ["30", "35", "49"], "这些数是 1²、2²、3²、4²、5²，下一个是 6²=36。", ["观察平方数。", "找到序号。", "计算 6²。"], "原创奥数-平方规律", "原创奥数题，与参考资料派生题隔离。"),
    text("orig-g4-olympiad-003", "一个三位数，百位比十位大 2，十位比个位大 2，三个数字和是 18。这个三位数是多少？", "864", "设个位为 x，则十位 x+2，百位 x+4，3x+6=18，x=4，所以是 864。", ["设个位。", "表示十位和百位。", "用数字和求解。"], "原创奥数-数字谜", "原创奥数题，与参考资料派生题隔离。")
  ]);

  add("g4-appendix", [
    choice("orig-g4-olympiad-004", "抢数游戏每次可报 1 到 3 个数，谁先报到 24 谁赢。后手想取胜，每轮应和对方合起来报几个数？", "4 个", ["3 个", "5 个", "6 个"], "24 是 4 的倍数，每轮凑 4 可以控制 4、8、12、16、20、24。", ["看每次最多 3。", "关键数是 3 + 1 = 4。", "每轮凑 4。"], "原创奥数-抢数策略", "原创奥数题，与参考资料派生题隔离。"),
    text("orig-g4-olympiad-005", "8 个点围成一圈，每相邻两点连线形成边，一共有多少条边？", "8", "围成一圈时，每个点对应一条到下一个点的边，共 8 条。", ["想成首尾相连。", "每个点连下一个点。", "共有 8 条边。"], "原创奥数-圆周计数", "原创奥数题，与参考资料派生题隔离。"),
    text("orig-g4-olympiad-006", "用 1、2、3、4 四个数字组成没有重复数字的四位数，其中偶数有多少个？", "12", "个位必须是 2 或 4，有 2 种；其余三位有 3×2×1 种，共 12 个。", ["先确定个位。", "再排列剩下三位。", "2 × 6 = 12。"], "原创奥数-排列偶数", "原创奥数题，与参考资料派生题隔离。")
  ]);

  add("g4-area", [
    text("orig-g4-olympiad-007", "一个长方形面积 96 平方厘米，长是宽的 3 倍。宽是多少厘米？", "不存在", "如果宽是整数，面积应是 3×宽×宽。检查 96 ÷ 3 = 32，不是平方数，所以不存在整数厘米的宽。", ["列出长是宽的 3 倍。", "把面积写成 3×宽×宽。", "检查 32 不是平方数。"], "原创奥数-条件审查", "原创奥数题，与参考资料派生题隔离。", ["不存在", "无整数解"] ),
    choice("orig-g4-olympiad-008", "把 12 厘米×8 厘米的长方形剪成尽量少的边长为整数厘米的小正方形，先剪最大的正方形边长应是多少？", "8 厘米", ["4 厘米", "6 厘米", "12 厘米"], "先剪 8×8 的最大正方形，剩下 4×8 再继续剪。", ["比较长和宽。", "最大正方形边长等于短边。", "选择 8 厘米。"], "原创奥数-正方形分割", "原创奥数题，与参考资料派生题隔离。")
  ]);

  add("g4-word", [
    text("orig-g4-olympiad-009", "甲乙两仓共有粮食 180 吨，甲给乙 20 吨后两仓一样多。甲原来有多少吨？", "110", "甲给乙 20 吨会让差减少 40 吨，原来甲比乙多 40 吨；(180 + 40) ÷ 2 = 110。", ["先求原来的差。", "用和差问题。", "求甲原来数量。"], "原创奥数-移多补少", "原创奥数题，与参考资料派生题隔离。"),
    choice("orig-g4-olympiad-010", "一项工程，甲 6 天完成，乙 12 天完成。两人合作几天完成？", "4 天", ["3 天", "6 天", "18 天"], "甲每天做 1/6，乙每天做 1/12，合做每天 1/4，所以 4 天完成。", ["写出每天效率。", "相加得到合作效率。", "用 1 除以合作效率。"], "原创奥数-工程问题", "原创奥数题，与参考资料派生题隔离。")
  ]);

  add("g4-statistics", [
    text("orig-g4-olympiad-011", "5 个数的平均数是 36，如果去掉一个数后，剩下 4 个数平均数是 34，被去掉的数是多少？", "44", "5 个数总和 180，剩下 4 个数总和 136，被去掉的是 44。", ["先求原总和。", "再求剩下总和。", "两者相减。"], "原创奥数-平均数还原", "原创奥数题，与参考资料派生题隔离。"),
    choice("orig-g4-olympiad-012", "一组数加入一个等于原平均数的数后，新的平均数会怎样？", "不变", ["一定变大", "一定变小", "变成 0"], "加入平均数本身，不会改变整体平均数。", ["理解平均数平衡点。", "新数等于平衡点。", "平均数不变。"], "原创奥数-平均数性质", "原创奥数题，与参考资料派生题隔离。")
  ]);
